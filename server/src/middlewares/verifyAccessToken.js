...
const verifyAccessToken = async (req, res, next) => {
    try {
        ...
        const user = User.findById(verifiedToken?.userId).select(
            '-password -__v'
        );
        ...
        if (error.message.includes('jwt expired')) {
            try {
                ...
                if (!verifiedRefreshToken) {
                    throw new ApiError(
                        statusCode.UNAUTHORIZED,
                        'Refresh token is not verified. Please Login!'
                    );
                }
                ...
                if (!user) {
                    throw new ApiError(
                        statusCode.UNAUTHORIZED,
                        'Error in fetching user! Kindly Login again!'
                    );
                }

                const newAccessToken =
                    await user.generateAccessTokenFromRefreshToken(
                        refreshToken
                    );
